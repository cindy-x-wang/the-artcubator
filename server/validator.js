...
// poke port 4444 to see if 'npm run hotloader' was possibly called
...
module.exports = {
  ...
  checkRoutes: (req, res, next) => {
    if (!routeChecked && req.url === "/") {
      // if the server receives a request on /, we must be on port 3000 not 4444
      if (!fs.existsSync("./client/dist/bundle.js")) {
        throw new NodeSetupError(
          "Couldn't find bundle.js! If you want to run the hot reloader, make sure 'npm run hotloader'\n" +
            "is running and then go to http://localhost:4444 instead of port 3000.\n" +
        ...
        if (active) {
          console.log(
            "Warning: It looks like 'npm run hotloader' may be running. Are you sure you don't want\n" +
              "to use the hot reloader? To use it, visit http://localhost:4444 and not port 3000"
          );
        }
      });
